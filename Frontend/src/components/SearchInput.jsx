...
function SearchInput({ placeholder }) {
  return (
    <div className="relative">
      <input
        className="w-full py-[10px] bg-transparent placeholder:text-text-700 dark:placeholder:text-slate-200 dark:text-slate-200  text-slate-700 text-sm border border-slate-200  rounded-md pl-3 pr-28  transition duration-300 ease focus:outline-none focus:border-slate-300 hover:border-slate-300 dark:hover:border-slate-700 shadow-sm focus:shadow"
      ...
      <button
        ...
        type="button"
      >
        <BiSearch className=" text-slate-400 dark:text-slate-200" />
        <span className=" text-slate-800 font-semibold"> جستجو</span>
      </button>
    </div>
  );
}
